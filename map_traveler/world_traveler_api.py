...
class WorldTravelerRec:
    ...
    async def find_exit(self):
        ...
        if self._cancel_token.is_cancelled:
            logger.info(f"Recursive traveler thread:[{self.id}] was cancelled.")
            return

        if exit_found:
            logger.info(
                f"Recursive traveler:[{self.id}] with origin:[{self._origin}] exited:[{self._exit}]. "
                f"Finished with status:[{exit_found} in {depth} steps.]")
        else:
            logger.info(
                f"Recursive traveler:[{self.id}] with origin:[{self._origin}] did not found the exit. "
                f"Finished with status:[{exit_found} in {depth} steps.]")

        feedback = {
            "type": "traveler_feedback",
            "traveler_id": self.id,
            "description": "Recursive traveler finished! Exit found!" if exit_found else "Recursive traveler finished! Exit not found"
        }
