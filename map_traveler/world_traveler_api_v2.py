...
from map_traveler.errors import TravelerFindingExitError
...
class WorldTravelerIter:
    ...
    async def find_exit(self):
        if self._finding_exit:
            raise TravelerFindingExitError()
        ...
        while not exit_found and move_index < len(directions) and not self._cancel_token.is_cancelled:
            ...
            exit_found, depth = await self._move_to(
                depth=1, origin=self._origin, direction=direction, destination=next_movement_coordinate
            )
        ...
        if self._cancel_token.is_cancelled:
            logger.info(f"Iterative traveler thread:[{self.id}] was cancelled.")
            return

        if exit_found:
            logger.info(
                f"Iterative traveler:[{self.id}] with origin:[{self._origin}] exited:[{self._exit}]. "
                f"Iterative traveler finished with status:[{exit_found} in {depth} steps.]")
        else:
            logger.info(
                f"Iterative traveler:[{self.id}] with origin:[{self._origin}] did not found the exit. "
                f"Iterative traveler finished with status:[{exit_found} in {depth} steps.]")

        feedback = {
            "type": "traveler_feedback",
            "traveler_id": self.id,
            "description": "Iterative traveler finished! Exit found!" if exit_found else "Iterative traveler finished! Exit not found"
        }
    ...
    async def _move_to(
            self, *, depth: int, direction: DirectionEnum, origin: Coordinate, destination: Coordinate = None
    ) -> Tuple[bool, int]:
        ...
        stack = [
            (
                ...
                origin,
                destination,
                await self._get_next_directions_dict(origin),
                0,
                MovementTypeEnum.Forward
            )
        ]
        ...
        while stack:
            ...
            depth, direction, origin, destination, movements_to_try, move_index, movement_type = stack.pop()
            ...
            if destination.row >= len(self._world) or destination.col >= len(self._world[0]):
                message = f"traveler: {self.id} found an exit in ({len(self._movements)})"
            ...
            if (
                    await self._is_out_of_bounds(destination) or
                    await self._is_not_available_spot_or_cell_is_visited_while_moving_forward(cell, movement_type)
            ):
                if await self._is_out_of_bounds(destination):
                    message = f"traveler: {self.id} trying to go ({str(destination)}) but is out of map"
                else:
                    message = f"traveler: {self.id} found a wall at ({str(destination)})"
                feedback.update({"description": message})
                self._call_back_progress(feedback)
                await self._handle_not_availability(
                    depth,
                    destination,
                    direction,
                    move_index,
            ...
            move_next = False
            while not move_next and not found and move_index < len(
                    movements_to_try) and not self._cancel_token.is_cancelled:
                while tried:
                ...
                # re-save stack step
                # stack.append((depth, direction, origin, destination, movements_to_try, move_index, movement_type))

                tried = True
                message = (
                    f"traveler: {self.id} trying to go from ({str(destination)}) to ({str(next_movement_coordinate)})"
                )
                feedback.update({"description": message})
                self._call_back_progress(feedback)
                ...
                depth += 1
                direction = next_direction_to_try
                new_origin = next_movement_coordinate
                next_movements_to_try = await self._get_next_directions_dict(new_origin)
                new_destination, _ = next_movements_to_try[direction]

                stack.append(
                    (depth, direction, new_origin, new_destination, next_movements_to_try, 0, MovementTypeEnum.Forward)
                )
    ...
    async def _is_not_available_spot_or_cell_is_visited_while_moving_forward(self, cell, movement_type):
        return not cell.is_available_spot or (self.id in cell.visited and movement_type == MovementTypeEnum.Forward)

    @staticmethod
    async def _is_out_of_bounds(destination):
        return destination.row < 0 or destination.col < 0

    @staticmethod
    async def _handle_not_availability(
            depth, destination, direction, move_index, movement_type, movements_to_try, origin, stack
    ):
        movements_to_try[direction] = (destination, True)
        tried = True
        next_direction_to_try = direction
        next_movement_coordinate, _ = movements_to_try[next_direction_to_try]
        while move_index < len(movements_to_try) and tried:
            next_direction_to_try = choice(list(movements_to_try.keys()))
            next_movement_coordinate, tried = movements_to_try[next_direction_to_try]
            move_index += 1
        if move_index == len(movements_to_try):
            for k in movements_to_try.keys():
                movements_to_try[k] = (movements_to_try[k][0], False)
            move_index = 0
            movement_type = MovementTypeEnum.Backward
        stack.append(
            (depth, next_direction_to_try, origin, next_movement_coordinate, movements_to_try, move_index,
             movement_type)
        )
        return next_direction_to_try, next_movement_coordinate
