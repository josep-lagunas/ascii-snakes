...
from map_traveler.world_traveler_api import WorldTravelerRec as WorldTravelerRec
from map_traveler.world_traveler_api_v2 import WorldTravelerIter
...
class RealTimeWebSocket(WebSocketHandler, ABC):
    ...
    async def on_message(self, message: Union[str, bytes]) -> Optional[Awaitable[None]]:
        ...
        if False:
            travelers_rec = [
                WorldTravelerRec.initialize_traveler(
                    str(i),
                    map_api.map,
                    snakes_size,
        ...
        travelers_iter = [
            WorldTravelerIter.initialize_traveler(
                str(i),
                map_api.map,
                snakes_size,
                call_traveler_progress,
            ...
            for i in range(int(travellers_count))
            if not self._cancel_token.is_cancelled
        ]
